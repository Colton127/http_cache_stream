...
class CachedResponseHeaders {
  ...
  static Future<CachedResponseHeaders> fromUrl(
    ...
    final response = await (httpClient?.head(url, headers: requestHeaders) ?? http.head(url, headers: requestHeaders));
  ...
  static Future<CachedResponseHeaders?> fromCacheFilesAsync(final CacheFiles cacheFiles) async {
    try {
      if (await cacheFiles.metadata.exists()) {
        final json = jsonDecode(await cacheFiles.metadata.readAsString());
        if (json is Map<String, dynamic>) {
          final headersFromJson = CachedResponseHeaders.fromJson(json['headers']);
          if (headersFromJson != null) return headersFromJson;
        }
      }
      return CachedResponseHeaders.fromFile(cacheFiles.complete, await cacheFiles.complete.stat());
    } catch (_) {
      return null;
    }
  }
  ...
  static CachedResponseHeaders? fromFile(final File file, [FileStat? fileStat]) {
